...
"""
function applyslices!(f::Function, out::AbstractVector, v::AbstractVector, slices; check::Bool=CHECK)
...
"""
function applyslices!(sf::Function, f::Function, out::AbstractVector, (idx,v)::PAIRVEC, τ; check::Bool=CHECK)
...
"""
function applyslices!(sf::Function, f::Function, out::AbstractVector, v::AbstractVector, τ; check::Bool=CHECK)
...
"""
function applyslices2!(f::Function, out::AbstractVector, v::AbstractVector, slices)
	@inbounds for (i,slice)=enumerate(slices)
...
"""
function applyslices(sf::Function, f::Function, (idx,v)::PAIRVEC{<:T}, τ; check::Bool=CHECK) where {T}
	check && @assert (issorted(idx) && (length(idx)==length(v)))
	slices = sf(idx, τ)
...
Apply `f` to slices over the implicit index (`eachindex(v)`) with slice function `sf`.
"""
function applyslices(sf::Function, f::Function, v::AbstractVector, τ; check::Bool=CHECK)
